...
def read_json_data(
    context, filepath, data_array_name, data_fields, encoding="utf-8-sig"
):
    ...
    report_message = 'Imported {num_values} from "{file_name}"'.format(
        num_values=i, file_name=file_name
    )
...
def read_csv_data(
    ...
    with open(filepath, "r", encoding=encoding, newline="") as csv_file:
        ...
        if len(mesh.vertices) > 0:
            create_object(mesh, object_name)
            report_message = 'Imported {num_values} lines from "{file_name}".'.format(
                num_values=i, file_name=file_name
            )
        else:
        ...
        report_message = "{message}\n{error_message}".format(
            message=report_message, error_message=error_message
        )
...
class ImportSpreadsheetData(bpy.types.Operator, ImportHelper):
    ...
    def execute(self, context):
        if self.filepath.endswith(".json"):
            ...
            report_message, report_type = read_csv_data(
                context,
                self.filepath,
                self.data_fields,
                self.csv_encoding,
                self.csv_delimiter,
                self.csv_leading_lines_to_discard,
            )
...
class RemoveDataFieldOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        operator.active_data_field_index = min(
            max(0, index - 1), len(operator.data_fields) - 1
        )
        return {"FINISHED"}
...
class SPREADSHEET_PT_json_options(bpy.types.Panel):
    ...
    def poll(cls, context):
        ...
        return (
            ...
            and operator.filepath.lower().endswith(".json")
        )
...
def menu_func_import(self, context):
    self.layout.operator(
        ImportSpreadsheetData.bl_idname, text="Spreadsheet Import (.csv, .json)"
    )
